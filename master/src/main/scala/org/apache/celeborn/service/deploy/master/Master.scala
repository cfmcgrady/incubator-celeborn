...
private[celeborn] class Master(
  ...
  if (conf.appQuotaEnabled) {
    logInfo("App quota is enabled in Master, start a thread pool to check quota.")
    quotaChecker.scheduleWithFixedDelay(
      new Runnable {
        override def run(): Unit = {
          try {
            val workerResourcesIter = statusSystem.workers.asScala.iterator.flatMap { workerInfo =>
              workerInfo.userResourceConsumption.asScala.iterator
            }
            quotaManager.refresh(workerResourcesIter)
          } catch {
            case t: Throwable => logError("Update app resource consumption failed.", t)
          }
        }
      },
      0L,
    ...
    masterSource.addGauge(MasterSource.JOB_EXCEED_QUOTA_COUNT) { () =>
      quotaManager.appQuotaStatus.size()
    }
  }
