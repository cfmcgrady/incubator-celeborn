...
| celeborn.master.appLevelResourceConsumption.metrics.enabled | false | Whether to enable app level resource consumption metrics. | 0.4.2.2 |  | 
...
| celeborn.master.http.maxWorkerThreads | 200 | Maximum number of threads in the master http worker thread pool. | 0.5.0 |  | 
...
| celeborn.master.http.stopTimeout | 5s | Master http server stop timeout. | 0.5.0 |  | 
| celeborn.master.port | 9097 | Port for master to bind. | 0.2.0 |  | 
| celeborn.master.slot.assign.extraSlots | 100 | Extra slots number when master assign slots. | 0.3.0 | celeborn.slots.assign.extraSlots | 
