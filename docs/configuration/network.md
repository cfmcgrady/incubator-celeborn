...
| celeborn.&lt;module&gt;.io.conflictAvoidChooser.enable | false | Whether to use conflict avoid event executor chooser in the client thread pool. If setting <module> to `rpc`, it works for shuffle client. If setting <module> to `data`, it works for shuffle client push and fetch data. If setting <module> to `replicate`, it works for replicate client of worker replicating data to peer worker. | 0.4.0 |  | 
