...
import org.apache.celeborn.common.protocol.{PartitionLocation, PartitionSplitMode, PartitionType, PbPushDataHandShake, PbPushMergedDataSplitPartitionInfo, PbRegionFinish, PbRegionStart}
  ...
  def handlePushMergedData(
    ...
    while (fileWriterIndex < totalFileWriters) {
      ...
      if (fileWriter == null) {
        ...
        if (fileWriter.isClosed) {
          logWarning(
            s"[handlePushMergedData] FileWriter is already closed! File path ${fileWriter.getFileInfo.getFilePath}")
